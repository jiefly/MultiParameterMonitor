...
public class RingView extends View {
    ...
    public volatile boolean mAnimationShowing;
    Path path = new Path();
    ...
    private void init() {
        ...
        if (!isInEditMode()) {
            // 造成错误的代码段
            mRingPaint.setColor(mContext.getResources().getColor(R.color.half_black));
        }
        ...
        if (!isInEditMode()) {
            mHeartBeatPaint.setColor(mContext.getResources().getColor(R.color.half_black));
        }
    ...
    public RingView setAnimationListener(OnAnimationListener animationListener) {
    ...
    public void stopAnim() {
        if (!mAnimationShowing) {
            return;
        }
        mAnimationShowing = false;
